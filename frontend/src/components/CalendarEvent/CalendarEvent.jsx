  ...
  const CalendarEvent = ({ event, onClick, onDragStart }) => {
    ...
    const calculateHeight = () => {
      ...
      return Math.max(Math.min(height, 64), 16); 
    };

    const colorClass =
      CategoryColors[event.category] || "bg-gray-200 border-gray-500";
    ...
    return (
      <div
        className={`relative w-full mx-1 p-1 border-l-4 rounded shadow-sm overflow-hidden hover:brightness-95 transition box-border ${colorClass} ${
          expanded ? "z-10" : ""
        }`}
        style={{
